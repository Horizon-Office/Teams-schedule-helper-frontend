...
export function ScheduleSidebar() {
  ...
  const activeTab: TabKey = pathname?.startsWith('/order') ? 'orders' : 'schedule';
  ...
  const allGroups = FACULTIES.flatMap(item => item.courses.flatMap(course => course.groups));
  ...
  const handleGroupPick = (match: GroupMatch) => {
    ...
    setOpenCourseIdsByFaculty(prev => {
      ...
      const next = current.includes(match.courseId)
        ? current
        : [...current, match.courseId];
